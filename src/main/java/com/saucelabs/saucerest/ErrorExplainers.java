...
class ErrorExplainers {
  ...
  static String missingCreds() {
    return String.join(
        ...
        " * You are using a CI platform which does not automatically propagate ENVars between"
            + " separate controller and processing hosts",
        " * You are running tests on an environment on which these properties are not set; A newly"
            + " build CI server, a Docker instance, etc");
  }
  ...
  static String videoMissing() {
    return String.join(
        ...
        " * This test was not able to complete video encoding due to an error or early"
            + " termination");
  }
  ...
  public static String HARMissing() {
    return String.join(
        System.lineSeparator(),
        " * This test was run without Extended Debugging. See"
            + " https://wiki.saucelabs.com/pages/viewpage.action?pageId=70072943",
        " * This test was not able to complete HAR file recording due to an error or early"
            + " termination");
  }
}
